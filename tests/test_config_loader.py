...
from httpmocker.config_loader import (
    Config,
    Endpoint,
...
class TestEndpoint:
    ...
    def test_missing_both_payloads(self):
        ...
        with pytest.raises(
            ValueError, match="Either payload_inline or payload_file must be specified"
        ):
            Endpoint(method="GET", path="/api/test", status=200, delay_ms=0)
...
class TestValidatePayloadFiles:
    ...
    def test_validate_existing_files(self):
        ...
        try:
            endpoint = Endpoint(
                method="GET", path="/api/test", status=200, delay_ms=0, payload_file=payload_path
            )
    ...
    def test_validate_missing_files(self):
        ...
        endpoint = Endpoint(
            method="GET", path="/api/test", status=200, delay_ms=0, payload_file="nonexistent.json"
        )
        config = Config(endpoints=[endpoint])
    ...
    def test_validate_inline_payloads_only(self):
        ...
        endpoint = Endpoint(
            method="GET", path="/api/test", status=200, delay_ms=0, payload_inline={"test": "data"}
        )
        config = Config(endpoints=[endpoint])
...
class TestEdgeCases:
    ...
    def test_endpoint_with_empty_path(self):
        ...
        endpoint = Endpoint(
            method="GET", path="", status=200, delay_ms=0, payload_inline={"root": True}
        )
    ...
    def test_config_with_multiple_endpoints(self):
        ...
        endpoints = [
            ...
            Endpoint(
                method="DELETE", path="/api/users/123", status=204, delay_ms=50, payload_inline={}
            ),
        ]
        config = Config(endpoints=endpoints)
